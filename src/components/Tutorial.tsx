...
export const Tutorial = ({show_tutorial, tutorial}: { show_tutorial: boolean; tutorial: TutorialType }) => {
  ...
  return(
    <div className={`Tutorial primary-content-container ${show_tutorial ? 'show' : 'hide'}`}>
      ...
      <div className='row'>
        <span id='previous_tutorial_button' className={`flat-button clickable ${tutorial.current_level_index > 0 ? null : 'locked'}`} onClick={previousTutorialButtonClicked}><span>← Previous</span></span>
        <span className='flat-button clickable' onClick={tutorialButtonClicked}><span>Play Game ►</span></span>
        <span id='next_tutorial_button' className={`flat-button clickable ${tutorial.current_level_index < tutorial.levels.length - 1 ? null : 'locked'}`} onClick={nextTutorialButtonClicked}><span>Next →</span></span>
      </div>
    </div>
  );
}
